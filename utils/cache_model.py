...
def cache_agent_ToM(model, rationality, agent_rewards, enforcer_actions, methods, cooperation_set):
    path = "cache/gridworld_" if GRIDWORLD == True else "cache/standard_"
    for method in methods:
        for cooperation in cooperation_set:
            filename = path + "agent_ToM_" + str(rationality) + "_" + method + "_" + str(cooperation) + "_" + str(NATURAL_COST) + \
                       ".csv"
            with open(filename, "w", newline="") as file:
                writer = csv.writer(file)
                data = [""] * (MAX_VALUE**(NUM_ACTIONS*2))
                for agent_reward in agent_rewards:
                    ...
                    for enforcer_action in enforcer_actions:
                        enforcer_action_index = "".join([str(num) for num in enforcer_action])
                        action_probabilities = model(rationality, agent_reward, enforcer_action, method=method, \
