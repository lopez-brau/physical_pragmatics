---
title: "Physical Pragmatics (Experiment 2b)"
output:
...
Here we analyze participant door endorsement ($N$=`r nrow(filter(data_2, door=="low_cost"))`, $M$=`r round(mean(filter(data_2, door=="low_cost")$age, na.rm=TRUE), 2)` years, $SD$=`r round(sd(filter(data_2, door=="low_cost")$age, na.rm=TRUE), 2)` years), collapsing across the door presentation order.
...
data_4 = data_2 %>%
  filter(door=="no_cost") %>%
  mutate(response=ifelse(response=="leave", 1, 0))
binom.test(x=sum(data_4$response), n=length(data_4$response), p=0.5,
