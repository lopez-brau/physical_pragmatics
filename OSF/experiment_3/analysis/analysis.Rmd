---
title: "Physical Pragmatics (Experiment 3)"
output:
...
First, we'll analyze the low-cost trial, which serves as a replication of Experiment 2a. Since the low-cost trials are identical across conditions, we analyze them jointly.
...
# trial, which is a replication of Experiment 2a.
plot_0 = data_7 %>%
...
# Compute the bootstrapped 95% CIs for participant confidence in the symbol
# trial across both conditions.
set.seed(seed)
ci = data.frame()
...
# Compute the participant confidence in the symbol trial across both
# conditions.
data_14 = data_5 %>%
  filter(trial %in% c("trial_4")) %>%
  mutate(target=as.numeric(target)) %>%
...
# Plot the participant confidence in the symbol trial across both conditions.
plot_2 = data_14 %>%
