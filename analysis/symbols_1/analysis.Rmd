...
compute_difference = function(data, indices) {
  congruent_data = mean(data[indices,]$congruent)
  incongruent_data = mean(data[indices,]$incongruent)
...
# Extract the individual participant confidence judgments per condition.
data_15 = data_5 %>%
  filter(condition=="congruent", trial=="trial_4") %>%
  select(target) %>%
  mutate(target=as.numeric(target)) %>%
  rename(congruent=target)
data_16 = data_5 %>%
  filter(condition=="incongruent", trial=="trial_4") %>%
  select(target) %>%
  mutate(target=as.numeric(target)) %>%
  rename(incongruent=target)
data_17 = bind_cols(data_15, data_16)
...
data_19 = data_5 %>%
  filter(condition=="incongruent", trial=="trial_4") %>%
  mutate(target=as.numeric(target))
wilcox.test(data_18$target, data_19$target)
```
...
set.seed(seed)
bootstrap_data = compute_bootstrap(data_20)
...
# across both conditions.
confidence = mean(data_20$target) * 100
...
data_21 = data_5 %>%
  ...
  spread(trial, target) %>%
  group_by(condition) %>%
  summarize(agreement=sum(trial_1==trial_3), total=n(),
...
```{r low-cost_trial_plot_excluded_participants, fig.align="center"}
# Filter the data from the low-cost trial.
data_22 = exclusions %>%
  filter(trial=="trial_1") %>%
  mutate(target=as.numeric(target))
...
```{r symbol_trial_plot_excluded_participants, fig.align="center"}
# Filter the data from the symbol trial.
data_24 = exclusions %>%
  filter(trial=="trial_3") %>%
  mutate(target=as.numeric(target))
...
# Plot the participant endorsement for the unmodified door in the symbol trial.
plot_4 = data_25 %>%
  ggplot(aes(x=condition, y=total, fill=target)) + 
  geom_bar(stat="identity", width=0.5, position="dodge") +
  theme_classic() +
  theme(aspect.ratio=1.0,
        legend.title=element_text(hjust=0.5)) +
...
```{r trial_agreement_excluded_participants}
# Compute the agreement between participant responses across trials in each
# condition.
data_26 = exclusions %>%
  filter(trial %in% c("trial_1", "trial_3")) %>%
