...
Because our exclusion question was potentially ambiguous, here we analyze the data from excluded participants to see if their judgments align with our main results.

```{r compute_endorsement_excluded_participants, fig.align="center"}
...
set.seed(seed)
ci = data.frame()
bootstrap_data = compute_bootstrap(filter(data_22, condition=="none")$response)
ci = rbind(ci, data.frame(lower_ci=bootstrap_data[4]*100,
                          upper_ci=bootstrap_data[5]*100,
                          condition="none"))

bootstrap_data = compute_bootstrap(filter(data_22, condition=="low")$response)
ci = rbind(ci, data.frame(lower_ci=bootstrap_data[4]*100,
                          upper_ci=bootstrap_data[5]*100,
                          condition="low"))
...
```{r compute_statistics_excluded_participants}
# Compute a binomial test for each condition.
# NOTE: Computing a two-sided binomial test for "conservativeness".
data_24 = filter(data_22, condition=="none")
binom.test(x=sum(data_24$response), n=length(data_24$response), p=0.5,
           alternative="two.sided")
data_25 = filter(data_22, condition=="low")
binom.test(x=sum(data_25$response), n=length(data_25$response), p=0.5,
           alternative="two.sided")
```
