...
data_0 %>%
  ...
  ggplot(aes(x=Actor_Reward, y=Enforcer_Action, group=factor(ToM))) + 
    ...
    xlab(TeX("Actor Preference (A - B)")) +
    scale_y_continuous(breaks=c(0:9)) +
    scale_x_continuous(breaks=c(-9:9)) +
    coord_cartesian(ylim=c(0,9)) +
    scale_color_manual(name="Theory of Mind", labels=c("None", "Some", "Full"), 
                       values=c("#F8766D","#00BA38", "#00BFC4"))

# Plot 2: Similar to Plot 1, except cooperation is set to 0 (i.e., the agent
...
# Plot 3: Plots the model predictions for how the enforcer's action changes as
...
data_0 %>%
  filter(Rationality==0.1, Method=="confidence", Cooperation==10,
         Enforcer_Reward==9, Actor_Reward %in% c(0, 3, 6, 9)) %>%
  ggplot(aes(x=ToM, y=Enforcer_Action, group=factor(Actor_Reward))) +
    geom_line(aes(color=factor(Actor_Reward)), position=position_dodge(0.08)) +
    geom_point(aes(color=factor(Actor_Reward)), size=3, position=position_dodge(0.08)) +
    theme_classic() +
    theme(plot.title=element_text(hjust=0.5)) +
    ylab("Enforcer Action") +
    xlab("Theory of Mind") +
    ...
    scale_color_manual(name="Actor Preference", 
                       ...
                       values=c("#F8766D","#00BA38", "#00BFC4", "#F564E3"))
