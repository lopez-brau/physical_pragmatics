...
def cache_dumb_enforcer(model, enforcer_rewards, rationality): 
	for enforcer_reward in enforcer_rewards:
...
def cache_smart_agent(model, agent_rewards, agent_cost, enforcer_actions, rationality, cooperation_set):
		...
		for cooperation in cooperation_set:
			for agent_reward in agent_rewards:
				for enforcer_action in enforcer_actions:
					action_probabilities = model(agent_reward, agent_cost, enforcer_action, rationality, cooperation, method, \
												 cache=True)
...
def access_dumb_enforcer(enforcer_reward):
...
def access_smart_agent(agent_reward, enforcer_action, cooperation, method):
	filename = "cache/smart_agent/" + method + "_" + str(cooperation) + "_" + \
			   ''.join([str(action_reward) for action_reward in agent_reward]) + "_" + \
			   ''.join([str(action) for action in enforcer_action]) + ".csv"
	with open(filename, "r") as file:
