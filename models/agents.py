...
def enforcer(rationality, enforcer_reward, p=0.0, method=None, cooperation=None, reward_assumptions=[], cache=False, \
			 plot=False):
	# Set up the utility space.
	space = tuple([GRIDWORLD_MAX_VALUE for action in np.arange(NUM_ACTIONS)]) if GRIDWORLD == True else \
			tuple([MAX_VALUE for action in np.arange(NUM_ACTIONS)])
	U = np.zeros(space)
	
	# Generate possible actor rewards and enforcer actions, taking into account any potential assumptions the enforcer 
	# may have about actor rewards.
	if SAMPLING == True:
	...
	if cache == True:
		...
		for actor_reward in actor_rewards:
			for enforcer_action in enforcer_actions:
				...
				if p != 0.0:
					actor_action_probabilities = actor_ToM(rationality, actor_reward, enforcer_action, method, \
														   cooperation, cache=True)
					expected_enforcer_reward = np.dot(enforcer_reward, actor_action_probabilities)
					temp_actor_ToM[tuple(enforcer_action)] = expected_enforcer_reward - \
															 (COST_RATIO*sum(enforcer_action))
...
def observer(infer, rationality, **kwargs):
	...
	# Infer the enforcer's beliefs about the cooperativeness of actors.
		...
		# Set up the space of possible cooperation parameters and the likelihood space.
		cooperation_set = kwargs["social_reward"][method]
	...
	# Infer the degree of ToM that the enforcer was acting for.
		...
		# Set up the space of possible proportion parameters and the likelihood space.
	...
	# Jointly infer what the enforcer's beliefs of the actor rewards and the degree of ToM that the enforcer was acting 
	# for.
		...
		for ar in np.arange(len(actor_rewards)):
			for p in np.arange(p_set.size):
				enforcer_action_probabilities = enforcer(rationality, enforcer_reward, p=p_set[p], method=method, \
														 ...
														 reward_assumptions=actor_rewards[ar], cache=True)
